...
def add_task():  
    task_str = task_entry.get()  
    if len(task_str) == 0:  
        messagebox.showinfo('Error', 'Field is Empty.')  
    else:    
        task_lst.append(task_str)   
        cursor.execute('insert into tasks values (?)', (task_str ,))    
        update_list()    
...
def delete_task():  
    try:  
        ...
        if the_value in task_lst:  
            task_lst.remove(the_value)    
            update_list()   
